...
export class HouseholdService extends BaseService {
  ...
  async addContainerToHousehold(containerId, householdId) {
    ...
    if (
      !(
        (await this.containerService.getContainerById(containerId)) ||
        !(await this.getHouseholdById(householdId))
      )
    )
      throw new Error(
        `Cannot create association between container: ${containerId} and household: ${householdId}`
      );
  ...
  async getContainersForHousehold(householdId) {
    this.__UseCollection(this.HOUSEHOLD_CONTAINER_COLLECTION);
    const containerHouseholdRelations = await this.__SearchForEntity({
      householdId,
    });
    ...
    for (const { containerId } of containerHouseholdRelations) {
      const containerObject = await this.containerService.getContainerById(
        containerId
      );
  ...
  async addGroceryListToHousehold(groceryListId, householdId) {
    try {
      if (
        !(
          (await this.groceryListService.getGroceryListById(groceryListId)) ||
          !(await this.getHouseholdById(householdId))
        )
      )
        throw new Error(
          `Cannont create association between groceryList: ${groceryListId} and household: ${householdId}`
        );
  ...
  async getGroceryListsForHousehold(householdId) {
    try {
      this.__UseCollection(this.HOUSEHOLD_GROCERY_LIST_COLLECTION);
      const groceryListHouseholdRelations = await this.__SearchForEntity({
        householdId,
      });

      const groceryLists = [];
      for (const { groceryListId } of groceryListHouseholdRelations) {
        const groceryListObject = await this.groceryListService.getGroceryListById(
          groceryListId
        );
